...
int main(){
    ...
    while(t--){
        ...
        // inner loop ko hatane ke liye hum half-length (len) par iterate karenge
        for(ll len = 1; len <= n / 2; len++){
            ll consecutive = 0;
            
            // s[i] aur s[i + len] ko compare karenge sliding window ki tarah
            for(ll i = 0; i < n - len; i++){
                if(s[i] == s[i + len] || s[i] == '?' || s[i + len] == '?'){
                    consecutive++;
                } else {
                    consecutive = 0; // Agar match nahi hua toh sequence toot gayi
                }

                // Agar consecutive matches 'len' ke barabar ho gaye, matlab tandem repeat mil gaya
                if(consecutive == len){
                    maxi = max(maxi, 2 * len);
                }
            }
